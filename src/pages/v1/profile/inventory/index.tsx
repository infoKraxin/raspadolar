...
const InventoryPage: React.FC = () => {
  ...
  const fixImageUrl = (url: string | null): string | null => {
    ...
    // Trocar raspa.ae por api.raspa.ae
    let fixedUrl = url.replace('https://raspa.ae/', 'https://api.raspa.ae/');
  ...
  const fetchPendingRedemptions = async () => {
    ...
    try {
      setLoading(true);
      const response = await fetch('https://api.raspa.ae/v1/api/users/redemptions/pending', {
        method: 'GET',
        headers: {
  ...
  const handleRedeem = async (gameId: string) => {
    ...
    try {
      const response = await fetch('https://api.raspa.ae/v1/api/users/redemptions/choose', {
        method: 'POST',
        headers: {
          'Authorization': `Bearer ${token}`,
          'Content-Type': 'application/json',
